...
<body id="page-top">
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    ...
                    <div class="d-sm-flex align-items-center justify-content-between mb-4">
                        <h1 class="h3 mb-0 text-gray-800">บันทึกข้อมูลสาเหตุคัดทิ้ง</h1>
                    ...
                    <div class="row">
                        ...
                        <div class="col-xl-12 col-lg-7">
                            <div class="card shadow mb-4">
                                ...
                                <table class="table table-bordered" id="causeTable" width="100%" cellspacing="0">
                                    <thead>
                                        <tr>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="addModalLabel">เพิ่มชื่อสาเหตุคัดทิ้ง</h5>
                ...
                <div class="modal-body">
                    <div class="form-group">
                        <label for="causeTypeName">ชื่อ</label>
                        <input type="text" class="form-control" id="causeTypeName" placeholder="" required>
                    </div>
                </div>
    ...
    <div class="modal fade" id="editModal" tabindex="-1" role="dialog" aria-labelledby="editModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="editModalLabel">แก้ไขชื่อสาเหตุคัดทิ้ง</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
    ...
    <script>
        $(document).ready(function() {
            var table = $('#causeTable').DataTable({
                "ajax": {
                    "url": "get_sortType.php",  // เรียกใช้ PHP ที่ดึงข้อมูล
                    ...
                    { "data": "SortTypeName" },
                    {
                        ...
                        "render": function (data, type, row) {
                            return '<div class="action-buttons">' +
                                   '<button class="edit-btn btn btn-warning shadow-sm" data-id="' + row.SortTypeID + '" data-name="' + row.SortTypeName + '"><i class="far fa-edit"></i></button>' +
                                   ' <button class="del-btn btn btn-danger shadow-sm" data-id="' + row.SortTypeID + '"><i class="far fa-trash-alt"></i></button>' +
                                   '</div>';
                        }
                    }
                ]
            ...
            $('#saveAddBtn').on('click', function() {
                var name = $('#causeTypeName').val();  // รับค่าจาก input field

                // ตรวจสอบให้แน่ใจว่ามีการป้อนชื่อ
                if (!name) {
                    Swal.fire('Error!', 'Please enter a cause type name.', 'error');
                    return;
                }

                // ส่งข้อมูลด้วย AJAX
                $.ajax({
                    url: 'sort_add.php',  // PHP ที่จัดการการเพิ่มข้อมูล
                    type: 'POST',
                    dataType: 'json',
                    data: {
                        name: name
                    ...
                    success: function(response) {
                        if (response.status === 'success') {
                            Swal.fire('Success!', response.message, 'success').then(() => {
                                ...
                                $('#causeTypeName').val('');   // ล้างค่า input field
                                $('#causeTable').DataTable().ajax.reload(null, false);  // รีเฟรช DataTable
                            });
                        } else {
            ...
            // เมื่อคลิกปุ่ม Edit
            $('#causeTable tbody').on('click', '.edit-btn', function() {
            ...
            $('#causeTable tbody').on('click', '.del-btn', function() {
                var id = $(this).data('id');
                Swal.fire({
                    title: 'Are you sure?',
